...
st.title("🌊 Water Quality Prediction Gradient Boosting")
st.image("grafik perbandingan metode.png", caption="Grafik Perbandingan Metode")
st.write('Dalam prediksi kali ini saya akan menggunakan model light gradient boosting machine, dimana fitur yang digunakan berjumlah 20 dengan akurasi pelatihan mencapai 95,13%.')
fitur = joblib.load('fiturgradientboosting.pkl')
st.write(fitur)
...
if st.button("Cek Prediksi"):
    scaler = joblib.load('saclergbc.pkl')
    ...
    # Load the model
    gbcmodel = joblib.load('modelgbc.pkl')

    # Make predictions
    prediksi = gbcmodel.predict(data_uji_scaled)
    if (prediksi == 0):

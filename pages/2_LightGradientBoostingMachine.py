...
st.set_page_config(
    page_title="Water Quality Prediction LightGBM",
    page_icon="🌊",
    layout="wide"
)

st.title("🌊 Water Quality Prediction LightGBM")
st.image("grafik perbandingan metode.png", caption="Grafik Perbandingan Metode")
st.write('Dalam prediksi kali ini saya akan menggunakan model light gradient boosting machine, dimana fitur yang digunakan berjumlah 20 dengan akurasi pelatihan mencapai 98,38%.')
fitur = joblib.load('fiturLightGradientBoostingMachine.pkl')
st.write(fitur)
...
if st.button("Cek Prediksi"):
    scaler = joblib.load('saclelightgbm.pkl')
    ...
    # Load the model
    lightgbmmodel = joblib.load('modellightgbm.pkl')

    # Make predictions
    prediksi = lightgbmmodel.predict(data_uji_scaled)
    if (prediksi == 0):
